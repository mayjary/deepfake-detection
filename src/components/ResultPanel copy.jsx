    ...
    <div className={`result-panel ${revealed ? "revealed" : ""} ${isFake ? "is-fake" : "is-real"}`}>
      ...
      <div className="result-meta">
        ...
        <div className="meta-row">
          ...
          <span className="meta-val">Deep-Fake-Detector-v2 (ViT)</span>
        </div>
        {result.details && (
          <>
            <div className="meta-row">
              <span className="meta-key">Raw FAKE</span>
              <span className="meta-val">{Math.round((result.details.raw_fake_prob || 0) * 100)}%</span>
            </div>
            <div className="meta-row">
              <span className="meta-key">Raw REAL</span>
              <span className="meta-val">{Math.round((result.details.raw_real_prob || 0) * 100)}%</span>
            </div>
            <div className="meta-row">
              <span className="meta-key">Threshold</span>
              <span className="meta-val">FAKE if &gt; {Math.round((result.details.threshold_used || 0.65) * 100)}%</span>
            </div>
          </>
        )}
        <div className="meta-row">
